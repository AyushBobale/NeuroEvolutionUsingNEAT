...
Add bigger impulsize force but with internal clocks
Normalize inputs
"""
...
class Human:
    def __init__(self, space, width, height):
        ...
        self.pos            = (100, 400)
        self.size           = (100, 150)
        self.thigh_size     = (0.25, 0.7)
        self.leg_size       = (0.25, 0.6)
        self.leg_pos        = 0.99      # defines how low the leg will be consequently how low the joint
        self.color          = (243, 168, 188, 100) #make colors derived that parts inherit colors
        ...
        self.not_capsized   = 1
        self.lowest_height  = (self.height -self.pos[1] - (self.size[1] * (1+self.thigh_size[1])) )/self.height
        #==========================================================================
    ...
    def getState(self):
        ...
        self.checkFitness()
        if self.lowest_height >  (self.height -self.pos[1] - (self.size[1] * (1+self.thigh_size[1])) )/self.height:
            self.lowest_height  = (self.height -self.pos[1] - (self.size[1] * (1+self.thigh_size[1])) )/self.height
        #+==========================================================
    ...
    def getFitness(self):
        ...
        return ((x+y)/(len(self.bodies)*2))/self.width * self.not_capsized * self.lowest_height
    ...
    def moveAI(self, ouput, force_multiplier = 1):
        ...
        if maxopid == 0:
            self.left_thigh_shape.body.apply_impulse_at_local_point((-force_multiplier, 0), (0,self.left_thigh_size[1]/2))
        if maxopid == 1:
            self.left_thigh_shape.body.apply_impulse_at_local_point((force_multiplier, 0), (0,self.left_thigh_size[1]/2))
        if maxopid == 2:
            self.left_leg_shape.body.apply_impulse_at_local_point((-force_multiplier, 0), (0,self.left_leg_size[1]/2))
        if maxopid == 3:
            self.left_leg_shape.body.apply_impulse_at_local_point((force_multiplier, 0), (0,self.left_leg_size[1]/2))

        if maxopid == 4:
            self.right_thigh_shape.body.apply_impulse_at_local_point((-force_multiplier, 0), (0,self.right_thigh_size[1]/2))
        if maxopid == 5:
            self.right_thigh_shape.body.apply_impulse_at_local_point((force_multiplier, 0), (0,self.right_thigh_size[1]/2))
        if maxopid == 6:
            self.right_leg_shape.body.apply_impulse_at_local_point((-force_multiplier, 0), (0,self.right_leg_size[1]/2))
        if maxopid == 7:
            self.right_leg_shape.body.apply_impulse_at_local_point((force_multiplier, 0), (0,self.right_leg_size[1]/2))
